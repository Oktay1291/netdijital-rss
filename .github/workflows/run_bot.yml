...
def generate_seo_article_and_labels(title, summary):
    ...
    allowed_categories = [
        "Yapay Zeka", "Telefon", "Bilgisayar", "Teknoloji", "Oyun", 
        "Akıllı Ev", "Donanım", "Bilim ve Uzay", "Sinema", "Otomobil"
    ]
...
def post_to_blogger():
    ...
    content = f"{article_html}<p><br></p><p><a href='{link}' target='_blank' rel='nofollow'>Haberi Kaynağından Oku</a></p>"
    post_body = {"title": title, "content": content, "labels": labels}

    try:
        request = service.posts().insert(blogId=BLOG_ID, body=post_body)
        request.execute()
        print(f"Yazı başarıyla yayınlandı: {title}")
    except Exception as e:
